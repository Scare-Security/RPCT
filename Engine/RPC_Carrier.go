...
type RPC_Data struct {
	Package_name         string
	Import_names_STDLIB  []string
	Import_VariablesFrom []string
	Body                 []string
	BodyFunctions        []string
}

var Template = `package %s // package name @{}@`
...
var (
	...
	LoadingVariablesBySTD  = regexp.MustCompile(`%{(.*)}%`)
	...
	RPCgenerateForWithVals = regexp.MustCompile(`(i?)RPC::ForLen(.*)`)
	RPCBOD                 RPC_Data
)
...
var LIA_Template = `
			func Generate_LIA_Tag(values string) string {
				var lias string
				lias += fmt.Sprintf("<li><a>%v</li></a>", values)
				return string(lias)
			}
`
